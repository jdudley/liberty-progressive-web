...
- `docs/framework.md` - **The canonical framework document.** The PDF
  (`dist/Liberty_Progressive_Framework_v<version>.pdf`) and the site's
  `framework.html` are build outputs of this file (`./build.sh`; requires node,
  pandoc, weasyprint). `framework.html` is generated by injecting the pandoc
  body into `framework.template.html` — never edit `framework.html` directly;
  edit the template for page chrome/styling, or `docs/framework.md` for
  content. Figures are generated from `figures/axes.json` and
